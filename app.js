...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (parseBulkBtn) {
        parseBulkBtn.addEventListener('click', () => {
            ...
            lines.forEach((line, index) => {
                ...
                if (/(?:^|\s|[^a-zA-Zа-яА-ЯёЁәіңғүұқөһӘІҢҒҮҰҚӨҺ])(?:adl|adults?|взросл[ыеяйах]*|взр|үлкен)(?=$|\s|[^a-zA-Zа-яА-ЯёЁәіңғүұқөһӘІҢҒҮҰҚӨҺ])/i.test(lowerLineForCat)) { parsedCategory = 'ADL'; }
                ...
                else if (/(?:^|\s|[^a-zA-Zа-яА-ЯёЁәіңғүұқөһӘІҢҒҮҰҚӨҺ])(?:snr|pensioners?|пенсионер[ыов]*|пенс|з[еи]й?неткер(?:лер)?)(?=$|\s|[^a-zA-Zа-яА-ЯёЁәіңғүұқөһӘІҢҒҮҰҚӨҺ])/i.test(lowerLineForCat)) { parsedCategory = 'SNR'; }
                ...
                const monthMap = {
                    'января': '01', 'январь': '01', 'янв': '01', 'февраля': '02', 'февраль': '02', 'фев': '02', 'марта': '03', 'март': '03', 'мар': '03', 'апреля': '04', 'апрель': '04', 'апр': '04', 'мая': '05', 'май': '05', 'июня': '06', 'июнь': '06', 'июн': '06', 'июля': '07', 'июль': '07', 'июл': '07', 'ввгуста': '08', 'август': '08', 'авг': '08', 'сентября': '09', 'сентябрь': '09', 'сен': '09', 'октября': '10', 'октябрь': '10', 'окт': '10', 'ноября': '11', 'ноябрь': '11', 'ноя': '11', 'декабря': '12', 'декабрь': '12', 'дек': '12', 'қаңтар': '01', 'кантар': '01', 'қаң': '01', 'ақпан': '02', 'акпан': '02', 'ақп': '02', 'наурыз': '03', 'нау': '03', 'сәуір': '04', 'сэуір': '04', 'сәу': '04', 'мамыр': '05', 'мам': '05', 'маусым': '06', 'мау': '06', 'шілде': '07', 'шилде': '07', 'шіл': '07', 'тамыз': '08', 'там': '08', 'қыркүйек': '09', 'кыркуйек': '09', 'қыр': '09', 'қазан': '10', 'казан': '10', 'қаз': '10', 'қараша': '11', 'караша': '11', 'қар': '11', 'желтоқсан': '12', 'желтоксан': '12', 'жел': '12', 'january': '01', 'jan': '01', 'february': '02', 'feb': '02', 'march': '03', 'mar': '03', 'april': '04', 'apr': '04', 'may': '05', 'june': '06', 'jun': '06', 'july': '07', 'jul': '07', 'august': '08', 'aug': '08', 'september': '09', 'sep': '09', 'october': '10', 'oct': '10', 'november': '11', 'nov': '11', 'december': '12', 'dec': '12'
                };
                ...
                for (let key of monthKeys) {
                    ...
                    if (regex.test(line)) { line = line.replace(regex, monthMap[key]); }
                }

                const dobRegexStr = /\b(0?[1-9]|[12]\d|3[01])([\.\-\/\s\,])(0?[1-9]|1[0-2])\2(\d{4}|\d{2})\b|\b(0?[1-9]|[12]\d|3[01])\.(0?[1-9]|1[0-2])(\d{4})\b|\b(0[1-9]|[12]\d|3[01])(0[1-9]|1[0-2])(\d{4}|\d{2})\b/;
                const dobMatch = line.match(dobRegexStr);
                let dobIso = ''; let matchedStr = '';
                if (dobMatch) {
                    matchedStr = dobMatch[0]; const parts = matchedStr.split(/[\.\-\/\s\,]+/);
                    let day = ''; let month = ''; let year = '';
                    if (parts.length >= 3) { day = parts[0].padStart(2, '0'); month = parts[1].padStart(2, '0'); year = parts[2]; }
                    else if (parts.length === 2) { day = parts[0].padStart(2, '0'); month = parts[1].slice(0, 2).padStart(2, '0'); year = parts[1].slice(2); }
                    else { day = matchedStr.slice(0, 2); month = matchedStr.slice(2, 4); year = matchedStr.slice(4); }
                    ...
                    if (ageMatch) { matchedStr = ageMatch[0]; tAge = parseInt(ageMatch[1], 10); dobIso = ''; }
                        ...
                        if (yearMatch) { matchedStr = yearMatch[0]; tYear = parseInt(yearMatch[1], 10); dobIso = ''; }
                            ...
                            if (simpleAgeMatch) { matchedStr = simpleAgeMatch[1]; tAge = parseInt(simpleAgeMatch[1], 10); dobIso = ''; }
                        }
                    }
                }
                ...
                namePart = namePart.replace(/\(?\b\d+\s*(?:жас|лет|год[а-я]*|yo|y\.o\.|years?|old)\)?/ig, '');
                namePart = namePart.replace(/\(\s*\d+\s*\)/g, '');
                namePart = namePart.replace(/(?:^|\s|[^a-zA-Zа-яА-ЯёЁәіңғүұқөһӘІҢҒҮҰҚӨҺ\'])(?:mr|mrs|ms|chd|inf|adl|snr|inv|pax|adults?|pensioners?|children|infants?)/ig, ' ');
                namePart = namePart.replace(/(?:^|\s|[^a-zA-Zа-яА-ЯёЁәіңғүұқөһӘІҢҒҮҰҚӨҺ\'])(?:взр(?:осл[а-я]*)?|реб(?:ен[окац]+)?|дети|дет(?:и|ям|ей|ях)?|млад(?:ен[а-я]*)?|пенс(?:ионер[а-я]*)?|инв(?:алид[а-я]*)?|зейнеткер(?:лер)?|мүгедек(?:тер)?|бала(?:лар)?|үлкен(?:дер)?)/ig, ' ');
                ...
                namePart = namePart.replace(/(?:^|\s|[^a-zA-Zа-яА-ЯёЁәіңғүұқөһӘІҢҒҮҰҚӨҺ\'])(?:билет|пассажир|итого|сумма|заявка|бронь|турист|тур|пакс)/ig, ' ');
                ...
                if (namePart.length >= 2 && !hasStrayNumbers && (!isSuspicious || isForced)) {
                    ...
                    const isDuplicate = tourists.some(t => t.fullName.toLowerCase() === touristObj.fullName.toLowerCase() && t.dob === touristObj.dob && t.age === touristObj.age && t.year === touristObj.year);
            ...
            if (unrecognizedLines.length > 0) {
                ...
                if (!isForced) { window.showToast(`Часть строк подозрительна. Если всё верно, нажмите еще раз.`, 'fa-triangle-exclamation', 'bg-amber-500'); }
                else { window.showToast(`Часть гостей не распознана (${unrecognizedLines.length} строк)`, 'fa-triangle-exclamation', 'bg-amber-500'); lastAttemptedText = ''; }
    ...
    function updateTouristDobDirect(id, value) {
        ...
        if (yearMatch) { tourist.year = parseInt(yearMatch[1], 10); tourist.dob = ''; delete tourist.age; delete tourist.category; delete tourist.categoryManuallySet; render(); return; }
        const ageRegex = /\b(\d{1,2})\b/;
        const ageMatch = value.match(ageRegex);
        if (ageMatch) { tourist.age = parseInt(ageMatch[1], 10); tourist.dob = ''; delete tourist.year; delete tourist.category; delete tourist.categoryManuallySet; render(); return; }
        tourist.dob = value; delete tourist.age; delete tourist.year; delete tourist.category; delete tourist.categoryManuallySet;
        render();
    }
    ...
    function updateTouristCategory(id, value) {
        ...
        if (tourist) {
            ...
            if (value !== 'INV') { tourist.disability = 'none'; } 
                ...
                if (age !== null && age >= 4 && age <= 11) { tourist.disability = '3'; } else { tourist.disability = '1'; }
            }
            render();
        }
    }
    ...
    function guessGender(name) {
        ...
        for (let word of words) {
            ...
            if (word.endsWith('айым') || word.endsWith('ару') || word.endsWith('аружан') || word.endsWith('улжан') || word.endsWith('ұлжан') || word.endsWith('асем') || word.endsWith('әсем') || word.endsWith('асель') || word.endsWith('әсел') || word.endsWith('айгерім') || word.endsWith('айгерим') || word.endsWith('арайлым')) { return 'female'; }
        ...
        for (let word of words) {
            if (word.length > 2 && (word.endsWith('а') || word.endsWith('я') || word.endsWith('a') || word.endsWith('ya'))) {
                if (!word.endsWith('овича') && !word.endsWith('евича') && !word.endsWith('ича') && !word.endsWith('илья') && !word.endsWith('никита') && !word.endsWith('данила') && !word.endsWith('баха')) { return 'female'; }
            }
        }
        return 'male';
    }
    ...
    function render() {
        ...
        if (visitDate && earlyBookingContainer) {
            const vDate = new Date(visitDate); const today = new Date();
            const isPromoDays = today.getMonth() === 5 && (today.getDate() >= 28 && today.getDate() <= 30);
            if (vDate.getMonth() === 6 && isPromoDays) { earlyBookingContainer.classList.remove('hidden'); } 
            else { earlyBookingContainer.classList.add('hidden'); if (earlyBookingToggle) earlyBookingToggle.checked = false; }
        }

        if (earlyBookingBadge && earlyBookingToggle) {
            if (earlyBookingToggle.checked) { earlyBookingBadge.classList.remove('hidden'); } else { earlyBookingBadge.classList.add('hidden'); }
        }
        ...
        tourists.forEach((t, index) => {
            ...
            if (t.age !== undefined) {
                age = t.age; const visitYear = visitDate ? new Date(visitDate).getFullYear() : new Date().getFullYear();
            ...
            else if (age === null && t.category) { category = t.category; }
            t.category = category;
            ...
            // ВЕРТИКАЛЬНАЯ ПРАВКА БАГА: Принудительные 50% скидки, если выставлен тип SNR (даже без возраста)
            if (category === 'SNR') {
                discountPercent = Math.max(discountPercent, 50);
                discountInfo.isPensioner = true;
            }
            ...
            if (t.fullName && (t.dob || t.age !== undefined || t.year !== undefined)) {
                ...
                let formattedDob = '';
                if (t.dob) { formattedDob = formatDate(t.dob); } 
                else if (t.year !== undefined) { formattedDob = `${t.year} г.`; } 
                else if (t.age !== undefined) { formattedDob = `${t.age} лет`; }

                exportDataList.push({ translitName: transliterate(t.fullName), category: category, formattedDob: formattedDob, tags: tags, gender: t.gender === 'female' ? 'F' : 'M', isBirthday: discountInfo.isBirthday });
            }
                ...
                <div class="col-span-12 w-full flex flex-wrap justify-between items-center mt-1 md:mt-0 md:contents border-t border-slate-100 md:border-0 pt-1.5 md:pt-0">
                    <div class="flex space-x-2 sm:space-x-4 md:space-x-6 md:contents">
                        ...
                        <div class="md:col-span-2 text-left md:text-center flex flex-col items-start md:items-center w-full md:w-auto">
                            ...
                            <select onchange="updateTourist('${t.id}', 'disability', this.value)" class="text-[9px] font-bold px-1.5 py-0.5 rounded border border-slate-200 text-slate-700 bg-white focus:outline-none transition-all duration-300 cursor-pointer text-center w-full md:w-auto">
                    ...
                    <div class="md:col-span-2 text-right flex flex-col items-end justify-center pr-2">
                        ...
                        <span class="text-xs font-bold ${finalPrice > 0 ? 'text-slate-900' : 'text-slate-400'}">${basePrice === -1 ? 'Нет тарифа' : Math.round(finalPrice).toLocaleString('ru-RU')} ₸</span>
                    </div>
                </div>
            `;
        ...
        if (!isTariffFound) { dateWarning.classList.remove('hidden'); } else { dateWarning.classList.add('hidden'); }
        ...
        if (currentCalcMode === 'quick') {
            ...
            if (!hasQuickGuests) { exportText += 'Пусто\n'; }
        } else {
        ...
        exportDataEl.value = exportText; exportDataEl.style.height = 'auto'; exportDataEl.style.height = exportDataEl.scrollHeight + 'px';

        const searchInput = document.getElementById('guestSearchInput');
        if (searchInput && searchInput.value) { if (window.filterGuests) { window.filterGuests(searchInput.value); } }

        saveDraft();

        window.updateTourist = updateTourist; window.removeTourist = removeTourist;
        window.updateTouristDobDirect = updateTouristDobDirect; window.updateTouristCategory = updateTouristCategory;
    }
    ...
    function syncDetailedToQuick() {
        ...
        tourists.forEach(t => {
            let age = null;
            if (t.age !== undefined) { age = t.age; } 
            else if (t.year !== undefined) { const visitYear = visitDate ? new Date(visitDate).getFullYear() : new Date().getFullYear(); age = visitYear - t.year; } 
            else { age = calculateAge(t.dob, visitDate); }
            let category = getPassengerCategory(age, t.gender, visitDate);
            if (t.categoryManuallySet && t.category) { category = t.category; } else if (age === null && t.category) { category = t.category; }

            if (t.disability === '1' || category === 'INV') { counts.inv++; } 
            else if (t.disability === '2') { counts.inv2++; } 
            else if (t.disability === '3') { counts.inv3++; } 
        ...
        quickCounts = counts; updateQuickInputsDOM();
    }

    function syncQuickToDetailed() {
        tourists = []; const today = new Date(); const visitDateStr = visitDateInput ? visitDateInput.value : '';
        const visitYear = visitDateStr ? new Date(visitDateStr).getFullYear() : today.getFullYear();
        
        for (let i = 0; i < quickCounts.adl; i++) { tourists.push({ id: createId(), fullName: `Гость ${tourists.length + 1}`, dob: `${visitYear - 25}-06-15`, gender: 'male', genderManuallySet: false, disability: 'none' }); }
        for (let i = 0; i < quickCounts.chld; i++) { tourists.push({ id: createId(), fullName: `Гость ${tourists.length + 1}`, dob: `${visitYear - 8}-06-15`, gender: 'male', genderManuallySet: false, disability: 'none' }); }
        for (let i = 0; i < quickCounts.pens; i++) { tourists.push({ id: createId(), fullName: `Гость ${tourists.length + 1}`, dob: `${visitYear - 65}-06-15`, gender: 'male', genderManuallySet: false, disability: 'none' }); }
        for (let i = 0; i < quickCounts.inf; i++) { tourists.push({ id: createId(), fullName: `Гость ${tourists.length + 1}`, dob: `${visitYear - 1}-06-15`, gender: 'male', genderManuallySet: false, disability: 'none' }); }
        for (let i = 0; i < quickCounts.inv; i++) { tourists.push({ id: createId(), fullName: `Гость ${tourists.length + 1}`, dob: `${visitYear - 30}-06-15`, gender: 'male', genderManuallySet: false, disability: '1', category: 'INV', categoryManuallySet: true }); }
        for (let i = 0; i < quickCounts.inv2; i++) { tourists.push({ id: createId(), fullName: `Гость ${tourists.length + 1}`, dob: `${visitYear - 30}-06-15`, gender: 'male', genderManuallySet: false, disability: '2', category: 'INV', categoryManuallySet: true }); }
        for (let i = 0; i < quickCounts.inv3; i++) { tourists.push({ id: createId(), fullName: `Гость ${tourists.length + 1}`, dob: `${visitYear - 30}-06-15`, gender: 'male', genderManuallySet: false, disability: '3', category: 'INV', categoryManuallySet: true }); }
        for (let i = 0; i < quickCounts.chld_inv; i++) { tourists.push({ id: createId(), fullName: `Гость ${tourists.length + 1}`, dob: `${visitYear - 8}-06-15`, gender: 'male', genderManuallySet: false, disability: '3', category: 'INV', categoryManuallySet: true }); }
    }

    function updateQuickInputsDOM() {
        const adlEl = document.getElementById('quick_adl'); const chldEl = document.getElementById('quick_chld');
        const pensEl = document.getElementById('quick_pens'); const infEl = document.getElementById('quick_inf');
        const invEl = document.getElementById('quick_inv'); const inv2El = document.getElementById('quick_inv2');
        const inv3El = document.getElementById('quick_inv3'); const chldInvEl = document.getElementById('quick_chld_inv');
        if (adlEl) adlEl.value = quickCounts.adl; if (chldEl) chldEl.value = quickCounts.chld; if (pensEl) pensEl.value = quickCounts.pens;
        if (infEl) infEl.value = quickCounts.inf; if (invEl) invEl.value = quickCounts.inv; if (inv2El) inv2El.value = quickCounts.inv2;
        if (inv3El) inv3El.value = quickCounts.inv3; if (chldInvEl) chldInvEl.value = quickCounts.chld_inv;
    }

    function switchCalcMode(mode) {
        currentCalcMode = mode;
        const tabDetailed = document.getElementById('tabDetailed'); const tabQuick = document.getElementById('tabQuick');
        const detailedModeContainer = document.getElementById('detailedModeContainer'); const quickModeContainer = document.getElementById('quickModeContainer');
        const detailedActionButtons = document.getElementById('detailedActionButtons'); const resetQuickBtn = document.getElementById('resetQuickBtn');
        const emptyState = document.getElementById('emptyState');
        if (!tabDetailed || !tabQuick) return;

        if (mode === 'detailed') {
            tabDetailed.classList.add('border-blue-500', 'text-blue-600'); tabDetailed.classList.remove('border-transparent', 'text-slate-400');
            tabQuick.classList.add('border-transparent', 'text-slate-400'); tabQuick.classList.remove('border-blue-500', 'text-blue-600');
            detailedModeContainer.classList.remove('hidden'); quickModeContainer.classList.add('hidden');
            detailedActionButtons.classList.remove('hidden'); resetQuickBtn.classList.add('hidden');
            if (tourists.length === 0 && (quickCounts.adl > 0 || quickCounts.chld > 0 || quickCounts.pens > 0 || quickCounts.inf > 0 || quickCounts.inv > 0)) { syncQuickToDetailed(); }
        } else {
            tabQuick.classList.add('border-blue-500', 'text-blue-600'); tabQuick.classList.remove('border-transparent', 'text-slate-400');
            tabDetailed.classList.add('border-transparent', 'text-slate-400'); tabDetailed.classList.remove('border-blue-500', 'text-blue-600');
            detailedModeContainer.classList.add('hidden'); quickModeContainer.classList.remove('hidden');
            detailedActionButtons.classList.add('hidden'); resetQuickBtn.classList.remove('hidden'); emptyState.classList.add('hidden');
            if (quickCounts.adl === 0 && quickCounts.chld === 0 && quickCounts.pens === 0 && quickCounts.inf === 0 && quickCounts.inv === 0 && quickCounts.inv2 === 0 && quickCounts.inv3 === 0) { syncDetailedToQuick(); }
        }
        render();
    }
    ...
    function changeQuickCount(category, delta) {
        if ((category === 'chld' || category === 'inf') && delta > 0 && quickCounts.adl === 0 && quickCounts.pens === 0) {
            if (window.showToast) { window.showToast('Дети могут посещать парк только в сопровождении взрослых', 'fa-triangle-exclamation', 'bg-amber-500'); } return;
        }
        quickCounts[category] = Math.max(0, quickCounts[category] + delta);
        if ((category === 'adl' || category === 'pens') && quickCounts.adl === 0 && quickCounts.pens === 0) { quickCounts.chld = 0; quickCounts.inf = 0; }
        updateQuickInputsDOM(); syncQuickToDetailed(); render();
    }
    ...
    function updateQuickCount(category, val) {
        ...
        if ((category === 'chld' || category === 'inf') && newVal > quickCounts[category] && quickCounts.adl === 0 && quickCounts.pens === 0) {
            if (window.showToast) { window.showToast('Дети могут посещать парк только в сопровождении взрослых', 'fa-triangle-exclamation', 'bg-amber-500'); } updateQuickInputsDOM(); return;
        }
    ...
    window.switchCalcMode = switchCalcMode; window.changeQuickCount = changeQuickCount;
    window.updateQuickCount = updateQuickCount; window.resetQuickCounts = resetQuickCounts;
    ...
    function validateAccompaniment() {
        ...
        if ((chldStat > 0 || infStat > 0) && adlStat === 0 && pensStat === 0) {
            if (window.showToast) { window.showToast('Внимание! Дети не могут быть в чеке без взрослых.', 'fa-triangle-exclamation', 'bg-red-500'); } return false;
        }
        return true;
    }

    function copyExportData() {
        if (!validateAccompaniment()) return; if (!exportDataEl.value) return;
        navigator.clipboard.writeText(exportDataEl.value).then(() => {
            ...
            copyExportBtn.classList.add('bg-emerald-600', 'text-white'); copyExportBtn.classList.remove('bg-blue-50', 'text-brand-blue');
            setTimeout(() => { copyExportBtn.innerHTML = originalHTML; copyExportBtn.classList.remove('bg-emerald-600', 'text-white'); copyExportBtn.classList.add('bg-blue-50', 'text-brand-blue'); }, 2000);
        });
    }
    ...
    if (downloadReceiptBtn) { downloadReceiptBtn.addEventListener('click', generateReceiptImage); }
    
    const nativeShareBtn = document.getElementById('nativeShareBtn');

    function getShareText() {
        if (!validateAccompaniment()) return ''; saveToHistory(); const text = exportDataEl.value; return `*Официальный расчет Tetys Blu*\n\n${text}`;
    }
    ...
    async function shareReceiptImage(btn) {
        ...
        try {
            ...
            if (navigator.canShare && navigator.canShare(shareData)) { await navigator.share(shareData); } else { throw new Error('ShareNotSupported'); }
        } catch (e) {
    ...
    if (nativeShareBtn) { nativeShareBtn.addEventListener('click', function() { shareReceiptImage(this); }); }
    ...
    if (whatsappShareBtn) {
        whatsappShareBtn.addEventListener('click', async function() {
            ...
            try {
                saveToHistory(); const { shareData } = await generateImageForShare(); const file = shareData.files[0]; let copied = false;
                if (navigator.clipboard && navigator.clipboard.write) { await navigator.clipboard.write([new ClipboardItem({ [file.type]: file })]); copied = true; }
                window.open('https://wa.me/', '_blank');
                if (copied) { window.showToast('Картинка скопирована! В WhatsApp нажмите "Вставить" (Paste)', 'fa-check', 'bg-green-600'); }
            } catch (err) {
                console.error('Ошибка копирования:', err);
                const text = getShareText();
                if (text) { window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank'); window.showToast('Браузер запретил копировать картинку. Отправлен текстовый чек.', 'fa-info-circle', 'bg-amber-500'); }
            } finally { this.innerHTML = originalHtml; }
        });
    }

    function sendToEmail() {
        if (!validateAccompaniment()) return; saveToHistory(); const text = exportDataEl.value; if (!text) return;
        const lines = text.split('\n'); let dateStr = ''; let tariffStr = ''; let bodyText = '';
        for (let i = 0; i < lines.length; i++) {
            if (lines[i].startsWith('Дата посещения:')) { dateStr = lines[i].trim(); } 
            else if (lines[i].startsWith('Тариф:')) { tariffStr = lines[i].trim(); } 
            else if (lines[i].startsWith('Список гостей:') || lines[i].startsWith('Состав гостей:')) { bodyText = lines.slice(i).join('\n').trim(); break; }
        }
        const subject = `${dateStr} | ${tariffStr}`;
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        if (isMobile) { window.location.href = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(bodyText)}`; } 
        else { window.open(`https://mail.google.com/mail/?view=cm&fs=1&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(bodyText)}`, '_blank'); }
    }
    
    const emailExportBtn = document.getElementById('emailExportBtn');
    if (emailExportBtn) { emailExportBtn.addEventListener('click', sendToEmail); }

    async function generateImageForShare() {
        return new Promise((resolve, reject) => {
            const container = document.getElementById('receiptContainer'); const content = document.getElementById('receiptContent');
            fillReceiptData();
            content.classList.remove('opacity-0', 'pointer-events-none'); document.body.appendChild(content); 
            content.style.position = 'fixed'; content.style.top = '0'; content.style.left = '0'; content.style.zIndex = '-9999';
            
            html2canvas(content, { scale: 2, backgroundColor: '#ffffff', logging: false }).then(canvas => {
                content.style.position = ''; content.style.top = ''; content.style.left = ''; content.style.zIndex = '';
                content.classList.add('opacity-0', 'pointer-events-none'); container.appendChild(content);
                ...
                canvas.toBlob(async (blob) => {
                    ...
                    const shareData = { files: [file] }; resolve({ shareData, dataUrl });
                }, 'image/png');
            }).catch(err => {
                content.style.position = ''; content.style.top = ''; content.style.left = '';
                content.classList.add('opacity-0', 'pointer-events-none'); container.appendChild(content); reject(err);
            });
        });
    }

    function fillReceiptData() {
        const metaEl = document.getElementById('receiptMeta'); const touristsEl = document.getElementById('receiptTourists');
        const dateParts = visitDateInput ? visitDateInput.value.split('-') : []; const visitDateStr = visitDateInput ? visitDateInput.value : '';
        const formattedDate = dateParts.length === 3 ? `${dateParts[2]}.${dateParts[1]}.${dateParts[0]}` : visitDateStr;
        const clientType = clientTypeInput ? clientTypeInput.value : 'tourist'; const tariffType = tariffTypeInput ? tariffTypeInput.value : 'day';
        ...
        if (currentCalcMode === 'quick') {
            let listHtml = '';
            if (quickCounts.adl > 0) { listHtml += `<div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-3"><div class="font-bold text-[#1e293b] text-[15px]">ВЗРОСЛЫЕ (ADL): ${quickCounts.adl}</div></div>`; }
            if (quickCounts.chld > 0) { listHtml += `<div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-3"><div class="font-bold text-[#1e293b] text-[15px]">ДЕТИ (CHLD): ${quickCounts.chld}</div></div>`; }
            if (quickCounts.pens > 0) { listHtml += `<div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-3"><div class="font-bold text-[#1e293b] text-[15px]">ПЕНСИОНЕРЫ (SNR): ${quickCounts.pens}</div></div>`; }
            if (quickCounts.inf > 0) { listHtml += `<div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-3"><div class="font-bold text-[#1e293b] text-[15px]">МЛАДЕНЦЫ (INF): ${quickCounts.inf}</div></div>`; }
            if (quickCounts.inv > 0) { listHtml += `<div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-3"><div class="font-bold text-[#1e293b] text-[15px]">ИНВАЛИДЫ (INV): ${quickCounts.inv}</div></div>`; }
            if (!listHtml) { listHtml = `<div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-3"><div class="font-bold text-slate-400 text-[15px]">СПИСОК ПУСТ</div></div>`; }
            touristsEl.innerHTML = listHtml;
        } else {
            tourists.forEach((t, i) => {
                if (!t.fullName && !t.dob && t.age === undefined && t.year === undefined) return;
                let age = null;
                if (t.age !== undefined) { age = t.age; } 
                else if (t.year !== undefined) { const visitYear = visitDateStr ? new Date(visitDateStr).getFullYear() : new Date().getFullYear(); age = visitYear - t.year; } 
                else { age = calculateAge(t.dob, visitDateStr); }

                let category = t.category;
                if (!t.categoryManuallySet || !category) { category = getPassengerCategory(age, t.gender, visitDateStr); }
                ...
                if (category === 'INV') { discountPercent = 100; }
                if (earlyBookingEnabled && !hasOtherDiscounts && discountPercent < 100 && age >= 4) {
                    discountPercent = Math.max(discountPercent, CONFIG.discounts.earlyBooking);
                }
                
                // ВЕРТИКАЛЬНАЯ ПРАВКА ДЛЯ ЧЕКА КАРТИНКОЙ:
                if (category === 'SNR') {
                    discountPercent = Math.max(discountPercent, 50);
                    discountInfo.isPensioner = true;
                }
                ...
                let formattedDob = '';
                if (t.dob) { formattedDob = formatDate(t.dob); } else if (t.year !== undefined) { formattedDob = `${t.year} г.`; } else if (t.age !== undefined) { formattedDob = `${t.age} лет`; }
                    ...
                    <div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-3 tourist-row">
                        <div class="flex-1 pr-4">
                            <div class="font-bold text-[#1e293b] text-[15px] leading-relaxed break-words">
                                ${(t.fullName || 'Гость ' + (i+1)).toUpperCase()} ${formattedDob ? '- ' + formattedDob : ''} <span class="text-xs text-slate-500 font-medium ml-1">(${category})</span>
                            </div>
                        </div>
                        <div class="text-right font-bold text-[#0076ba] text-[15px] shrink-0">${priceStr}</div>
                    </div>
                `;
            });
        }
        const receiptTotalValue = document.getElementById('receiptTotalValue');
        if (receiptTotalValue && totalPriceEl) { receiptTotalValue.textContent = totalPriceEl.textContent; }
    }

    function generateReceiptImage() {
        if (!validateAccompaniment()) return; saveToHistory();
        const container = document.getElementById('receiptContainer'); const content = document.getElementById('receiptContent');
        const formattedDate = visitDateInput ? visitDateInput.value : 'date';
        fillReceiptData();
        content.classList.remove('opacity-0', 'pointer-events-none'); document.body.appendChild(content); 
        content.style.position = 'fixed'; content.style.top = '0'; content.style.left = '0'; content.style.zIndex = '-9999';
        const originalBgHtml = downloadReceiptBtn.innerHTML; downloadReceiptBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin mr-1.5"></i> Создание...';
        
        html2canvas(content, { scale: 2, backgroundColor: '#ffffff' }).then(canvas => {
            content.style.position = ''; content.style.top = ''; content.style.left = ''; content.style.zIndex = '';
            content.classList.add('opacity-0', 'pointer-events-none'); container.appendChild(content);
            const link = document.createElement('a'); link.download = `TetysBlu_Check_${formattedDate}.png`; link.href = canvas.toDataURL('image/png'); link.click();
            downloadReceiptBtn.innerHTML = originalBgHtml; window.showToast('Чек успешно сохранен!', 'fa-circle-check');
        }).catch(err => {
            console.error('Ошибка создания чека', err); downloadReceiptBtn.innerHTML = originalBgHtml; window.showToast('Ошибка при создании чека', 'fa-triangle-exclamation', 'bg-red-500');
            content.style.position = ''; content.style.top = ''; content.style.left = ''; content.classList.add('opacity-0', 'pointer-events-none'); container.appendChild(content);
        });
    }

    // --- ИСТОРИЯ РАСЧЕТОВ ---
    const historyBtn = document.getElementById('historyBtn'); const closeHistoryBtn = document.getElementById('closeHistoryBtn');
    const historyModal = document.getElementById('historyModal'); const historyModalContent = document.getElementById('historyModalContent');
    ...
    function saveToHistory() {
        ...
        const record = { id: Date.now(), timestamp: new Date().toISOString(), visitDate: visitDateInput ? visitDateInput.value : '', clientType: clientTypeInput ? clientTypeInput.value : 'tourist', tariffType: tariffTypeInput ? tariffTypeInput.value : 'day', totalSum: total, tourists: JSON.parse(JSON.stringify(tourists)) };
        let history = JSON.parse(localStorage.getItem('tetysBluHistory') || '[]');
        if (history.length > 0) { const last = history[0]; if (JSON.stringify(last.tourists) === JSON.stringify(record.tourists) && last.visitDate === record.visitDate) { return; } }
        history.unshift(record); if (history.length > 20) history = history.slice(0, 20); localStorage.setItem('tetysBluHistory', JSON.stringify(history));
    }

    if (historyBtn) {
        historyBtn.addEventListener('click', () => {
            renderHistory(); historyModal.classList.remove('hidden');
            setTimeout(() => { historyModal.classList.remove('opacity-0'); historyModalContent.classList.remove('translate-x-full'); }, 10);
        });
    }
    if (closeHistoryBtn) {
        closeHistoryBtn.addEventListener('click', () => {
            historyModal.classList.add('opacity-0'); historyModalContent.classList.add('translate-x-full');
            setTimeout(() => { historyModal.classList.add('hidden'); }, 300);
        });
    }

    function renderHistory() {
        if (!historyList) return; const history = JSON.parse(localStorage.getItem('tetysBluHistory') || '[]');
        if (history.length === 0) { historyList.innerHTML = '<div class="text-center text-slate-400 py-10"><i class="fa-solid fa-folder-open text-3xl mb-3 opacity-50"></i><p class="text-sm font-semibold">Архив пуст</p></div>'; return; }
        historyList.innerHTML = '';
        history.forEach(item => {
            const date = new Date(item.timestamp); const timeStr = `${date.getDate().toString().padStart(2,'0')}.${(date.getMonth()+1).toString().padStart(2,'0')} в ${date.getHours().toString().padStart(2,'0')}:${date.getMinutes().toString().padStart(2,'0')}`;
            const card = document.createElement('div'); card.className = 'bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col space-y-2 relative';
            card.innerHTML = `
                <div class="flex justify-between items-center"><span class="text-[10px] font-bold text-slate-400 uppercase">${timeStr}</span><span class="text-xs font-black text-[#1e293b]">${item.totalSum.toLocaleString('ru-RU')} ₸</span></div>
                ...
                <button class="mt-2 w-full bg-blue-50 text-brand-blue hover:bg-brand-blue hover:text-white py-2 rounded-xl text-xs font-bold transition-colors"><i class="fa-solid fa-download mr-1.5"></i>Загрузить расчет</button>
            `;
            const loadBtn = card.querySelector('button');
            loadBtn.addEventListener('click', () => {
                if (visitDateInput) visitDateInput.value = item.visitDate; if (clientTypeInput) clientTypeInput.value = item.clientType; if (tariffTypeInput) tariffTypeInput.value = item.tariffType;
                tourists = item.tourists; render(); window.showToast('Расчет успешно загружен', 'fa-folder-open', 'bg-brand-blue'); closeHistoryBtn.click();
            });
    ...
    // --- PWA LOGIC ---
    let deferredPrompt; const installPwaBtn = document.getElementById('installPwaBtn');
    window.addEventListener('beforeinstallprompt', (e) => { e.preventDefault(); deferredPrompt = e; if (installPwaBtn) installPwaBtn.classList.remove('hidden'); });
    if (installPwaBtn) { installPwaBtn.addEventListener('click', async () => { if (deferredPrompt) { deferredPrompt.prompt(); const { outcome } = await deferredPrompt.userChoice; if (outcome === 'accepted') { installPwaBtn.classList.add('hidden'); } deferredPrompt = null; } }); }
    window.addEventListener('appinstalled', () => { if (installPwaBtn) installPwaBtn.classList.add('hidden'); window.showToast('Приложение установлено!', 'fa-check', 'bg-emerald-500'); });

    // Инициализатор приложения (Заглушка/Стартовый вызов)
    function initApp() {
        ...
        if (draft) {
            try {
                ...
                if (data.tourists && Array.isArray(data.tourists) && data.tourists.length > 0) { tourists = data.tourists; } else { addTourist(); }
                if (data.currentCalcMode) currentCalcMode = data.currentCalcMode;
                if (data.quickCounts) quickCounts = data.quickCounts;
                setTimeout(() => { switchCalcMode(currentCalcMode); }, 50);
            } catch (e) { addTourist(); }
        } else { addTourist(); }
        render();
    }
